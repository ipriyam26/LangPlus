...
import langplus
from langplus.cache import GPTCache
...
def test_gptcache_caching(
    ...
    langplus.llm_cache = GPTCache(init_func)
    llm = FakeLLM()
    params = llm.dict()
    params["stop"] = None
    ...
    cache_output = langplus.llm_cache.lookup("foo", llm_string)
    assert cache_output == [Generation(text="fizz")]

    langplus.llm_cache.clear()
    assert langplus.llm_cache.lookup("bar", llm_string) is None
